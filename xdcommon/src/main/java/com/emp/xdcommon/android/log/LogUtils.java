package com.emp.xdcommon.android.log;
...
public class LogUtils {
    ...
    private static String LOGDIR= SdCardUtil.getNormalSDCardPath()+File.separator+"Log";
    ...
    public static void logConfigure(Context mcontext,String logDir) {
        context=mcontext;
        LOGDIR = logDir;
        File mkFile = new File(LOGDIR);
        if (!mkFile.exists())
            Log.v(TAG, "创建Log路径：" + mkFile.mkdirs());
    }
...
//                Log.e("raf","raf:"+logInfo.raf.getFD().valid());
                if (logInfo.raf != null &&new File(logInfo.path).exists()&&logInfo.raf.length() < MAX_FILE_SIZE) {
                } else {
                    int index = 0;
                    ...
                    logInfo.index = index;
                    logInfo.raf = raf;
                    logInfo.path=logFile.getAbsolutePath();
                }
            ...
            if (logInfo.raf != null) {
                ...
                writeTxtToFile("[" + time + "]" + "[" + tag + "]" + "["+ PackageUtil.getAppVersionName(context)+"]"+message, logInfo.raf);
            }
        }
        ...
        try {
            if (LEVEL <= VERBOSE) {
                ...
                if(stackTrace.length>0){
                    ...
                    stackTraceElement4 = stackTrace[stackTrace.length-1];
                }
